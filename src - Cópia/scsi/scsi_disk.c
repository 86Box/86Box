...
scsi_disk_set_phase(scsi_disk_t *dev, uint8_t phase)
{
    ...
    uint8_t scsi_lun = dev->drv->scsi_lun;
...
{
    if (((dev->request_length >> 5) & 7) != dev->drv->scsi_lun) {
...
scsi_disk_command(scsi_disk_t *dev, uint8_t *cdb)
{
    ...
    hdbufferb = SCSIDevices[dev->drv->scsi_id][dev->drv->scsi_lun].CmdBuffer;
    BufLen = &SCSIDevices[dev->drv->scsi_id][dev->drv->scsi_lun].BufferLength;
...
{
    uint8_t *hdbufferb = SCSIDevices[dev->drv->scsi_id][dev->drv->scsi_lun].CmdBuffer;
    int32_t *BufLen = &SCSIDevices[dev->drv->scsi_id][dev->drv->scsi_lun].BufferLength;
