...
static int64_t max_spt = 63, max_hpc = 255, max_tracks = 266305;
static uint64_t mfm_tracking, esdi_tracking, xta_tracking, ide_tracking, scsi_tracking[16];
static uint64_t size, selection = 127;
static int net_ignore_message = 0, next_free_id = 0;
static int hd_listview_items, hdc_id_to_listview_index[HDD_NUM];
static int hdlv_current_sel, hard_disk_added = 0;
static int no_update = 0, existing = 0, chs_enabled = 0;
static int spt, hpc, tracks, ignore_change = 0;
static int fdlv_current_sel, cdlv_current_sel, zdlv_current_sel;
static int fd_ignore_change = 0, rd_ignore_change = 0;
...
win_settings_floppy_drives_proc(HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		if ((((LPNMHDR)lParam)->code == LVN_ITEMCHANGED) && (((LPNMHDR)lParam)->idFrom == IDC_LIST_FLOPPY_DRIVES)) {
			...
			fdlv_current_sel = get_selected_drive(hdlg, IDC_LIST_FLOPPY_DRIVES);
...
win_settings_other_removable_devices_proc(HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		if ((((LPNMHDR)lParam)->code == LVN_ITEMCHANGED) && (((LPNMHDR)lParam)->idFrom == IDC_LIST_CDROM_DRIVES)) {
			...
			cdlv_current_sel = get_selected_drive(hdlg, IDC_LIST_CDROM_DRIVES);
			...
			zdlv_current_sel = get_selected_drive(hdlg, IDC_LIST_ZIP_DRIVES);
