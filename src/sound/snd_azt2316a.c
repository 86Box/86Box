...
*/
		...
		mpu401_setaddr(azt2316a->mpu, azt2316a->cur_mpu401_addr);
...
static void *
azt2316a_init(const device_t *info)
{
	FILE *f;
        int i;
        ...
        /* config */
        f = nvr_fopen(L"azt2316a.nvr", L"rb");
        if (f) {
                uint8_t checksum = 0;
                uint8_t saved_checksum;

                /* decoded data saved for convenience */
                fread(&azt2316a->cur_addr, sizeof(azt2316a->cur_addr), 1, f);
                checksum += azt2316a->cur_addr * 2;

                fread(&azt2316a->cur_irq, sizeof(azt2316a->cur_irq), 1, f);
                checksum += azt2316a->cur_irq * 3;

                fread(&azt2316a->cur_dma, sizeof(azt2316a->cur_dma), 1, f);
                checksum += azt2316a->cur_dma * 5;

                fread(&azt2316a->cur_wss_enabled, sizeof(azt2316a->cur_wss_enabled), 1, f);
                checksum += azt2316a->cur_wss_enabled * 7;

                fread(&azt2316a->cur_wss_addr, sizeof(azt2316a->cur_wss_addr), 1, f);
                checksum += azt2316a->cur_wss_addr * 11;

                fread(&azt2316a->cur_wss_irq, sizeof(azt2316a->cur_wss_irq), 1, f);
                checksum += azt2316a->cur_wss_irq * 13;

                fread(&azt2316a->cur_wss_dma, sizeof(azt2316a->cur_wss_dma), 1, f);
                checksum += azt2316a->cur_wss_dma * 17;

                fread(&azt2316a->cur_mpu401_addr, sizeof(azt2316a->cur_mpu401_addr), 1, f);
                checksum += azt2316a->cur_mpu401_addr * 19;

                fread(&azt2316a->cur_mpu401_irq, sizeof(azt2316a->cur_mpu401_irq), 1, f);
                checksum += azt2316a->cur_mpu401_irq * 23;

                fread(&azt2316a->cur_mode, sizeof(azt2316a->cur_mode), 1, f);
                checksum += azt2316a->cur_mode * 29;

                /* load the eeprom itself */
                fread(read_eeprom, AZTECH_EEPROM_SIZE, 1, f);
                for (i = 0; i < AZTECH_EEPROM_SIZE; i++)
                        checksum += read_eeprom[i];

                fread(&saved_checksum, sizeof(saved_checksum), 1, f);
	...
	if (!loaded_from_eeprom) {
		azt2316a->cur_addr = device_get_config_hex16("addr");
...
azt2316a_close(void *p)
{
	...
	FILE *f;
        ...
        f = nvr_fopen(L"azt2316a.nvr", L"wb");
        if (f) {
                int i;

                /* save decoded data for convenience when starting up TODO: confirm any models saving mixer settings to EEPROM */
                checksum += azt2316a->cur_addr * 2;
                fwrite(&azt2316a->cur_addr, sizeof(azt2316a->cur_addr), 1, f);

                checksum += azt2316a->cur_irq * 3;
                fwrite(&azt2316a->cur_irq, sizeof(azt2316a->cur_irq), 1, f);

                checksum += azt2316a->cur_dma * 5;
                fwrite(&azt2316a->cur_dma, sizeof(azt2316a->cur_dma), 1, f);

                checksum += azt2316a->cur_wss_enabled * 7;
                fwrite(&azt2316a->cur_wss_enabled, sizeof(azt2316a->cur_wss_enabled), 1, f);

                checksum += azt2316a->cur_wss_addr * 11;
                fwrite(&azt2316a->cur_wss_addr, sizeof(azt2316a->cur_wss_addr), 1, f);

                checksum += azt2316a->cur_wss_irq * 13;
                fwrite(&azt2316a->cur_wss_irq, sizeof(azt2316a->cur_wss_irq), 1, f);

                checksum += azt2316a->cur_wss_dma * 17;
                fwrite(&azt2316a->cur_wss_dma, sizeof(azt2316a->cur_wss_dma), 1, f);

                checksum += azt2316a->cur_mpu401_addr * 19;
                fwrite(&azt2316a->cur_mpu401_addr, sizeof(azt2316a->cur_mpu401_addr), 1, f);

                checksum += azt2316a->cur_mpu401_irq * 23;
                fwrite(&azt2316a->cur_mpu401_irq, sizeof(azt2316a->cur_mpu401_irq), 1, f);

                checksum += azt2316a->cur_mode * 29;
                fwrite(&azt2316a->cur_mode, sizeof(azt2316a->cur_mode), 1, f);

                /* save the eeprom itself */
                for (i = 0; i < AZTECH_EEPROM_SIZE; i++)
                        checksum += azt2316a->sb->dsp.azt_eeprom[i];
                fwrite(azt2316a->sb->dsp.azt_eeprom, AZTECH_EEPROM_SIZE, 1, f);

                /* TODO: should remeber to save wss duplex setting if it's implemented in the future */
...
{
        {
                ...
                {
                        {
                                .description = "Sound Galaxy Pro 16 AB/MMPRO16AB (Washington family)",
                                .value = SB_SUBTYPE_CLONE_AZT2316A_0X11
                        },
                        {
                                .description = "Sound Galaxy Nova 16 Extra, Packard Bell Forte 16/FORTE16 (Clinton family)",
                                .value = SB_SUBTYPE_CLONE_AZT1605_0X0C
                        },
                },
                .default_int = SB_SUBTYPE_CLONE_AZT2316A_0X11
        },
        {
                .name = "codec",
                .description = "CODEC",
        ...
        {
                "addr", "SB Address", CONFIG_HEX16, "", 0x220,
                {
                        {
                                "0x220", 0x220
                        },
	...
	{
		"wss_enabled", "Enable WSS", CONFIG_BINARY, "", 1
	},
        {
                "wss_addr", "WSS Address", CONFIG_HEX16, "", 0x530,
                {
                        {
                                "0x530", 0x530
                        },
	...
	{
		"wss_dma", "WSS DMA", CONFIG_SELECTION, "", 0,
		{
			{
				"0", 0
			},
        ...
        {
                "mpu401_addr", "MPU-401 Address", CONFIG_HEX16, "", 0x330,
                {
                        {
                                "0x300", 0x300
                        },
        ...
        {
                "mpu401_irq", "MPU-401 IRQ", CONFIG_SELECTION, "", 2,
                {
                        {
                                "IRQ 2/9", 2
                        },
...
const device_t azt2316a_device =
{
	...
	0,
        azt2316a_init, azt2316a_close, NULL, NULL,
