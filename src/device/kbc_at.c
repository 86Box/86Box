...
static uint8_t
write64_umc(void *priv, uint8_t val)
{
    atkbc_t *dev     = (atkbc_t *) priv;
    uint8_t  kbc_ven = dev->flags & KBC_VEN_MASK;
...
static uint8_t
write60_award(void *priv, uint8_t val)
{
    atkbc_t *dev = (atkbc_t *) priv;
...
static uint8_t
write64_award(void *priv, uint8_t val)
{
    atkbc_t *dev     = (atkbc_t *) priv;
    uint8_t  kbc_ven = dev->flags & KBC_VEN_MASK;
...
static uint8_t
write60_chips(void *priv, uint8_t val)
{
    atkbc_t *dev = (atkbc_t *) priv;
...
static uint8_t
write64_chips(void *priv, uint8_t val)
{
    atkbc_t *dev     = (atkbc_t *) priv;
    uint8_t  kbc_ven = dev->flags & KBC_VEN_MASK;
...
write64_phoenix(void *priv, uint8_t val)
{
    ...
    switch (val) {
        ...
        /* NOTE: The MultiKey/42i reference does not document these at all.
                 ...
                 removed, so these may have been amogn them, and we have no
...
kbc_at_init(const device_t *info)
{
    ...
    switch (dev->flags & KBC_VEN_MASK) {
        ...
        case KBC_VEN_VIA:
            ...
            dev->write60_ven = write60_award;
            dev->write64_ven = write64_award;
            break;
        ...
        case KBC_VEN_UMC:
            ...
            dev->write64_ven = write64_umc;
            break;

        case KBC_VEN_CHIPS:
            if ((info->local & 0xff00) != 0x0000)
                kbc_chips_revision = (info->local >> 8) & 0xff;
            dev->write60_ven = write60_chips;
            dev->write64_ven = write64_chips;
            break;
...
const device_t keyboard_at_holtek_device = {
...
const device_t keyboard_at_umc_device = {
...
const device_t keyboard_at_award_device = {
...
const device_t keyboard_at_chips_device = {
...
const device_t keyboard_at_quadtel_device = {
...
const device_t keyboard_at_via_device = {
...
const device_t keyboard_ps2_umc_device = {
...
const device_t keyboard_ps2_award_device = {
...
const device_t keyboard_ps2_via_device = {
