...
serial_update_ints(serial_t *dev)
{
    ...
    if ((dev->ier & 0x04) && (dev->int_status & SERIAL_INT_LSR)) {
        /* Line status interrupt */
        stat     = 1;
        dev->iir = 0x06;
    } else if ((dev->ier & 0x01) && (dev->int_status & SERIAL_INT_TIMEOUT)) {
        /* Received data available */
        stat     = 1;
        dev->iir = 0x0c;
    } else if ((dev->ier & 0x01) && (dev->int_status & SERIAL_INT_RECEIVE)) {
        /* Received data available */
        stat     = 1;
        dev->iir = 0x04;
    } else if ((dev->ier & 0x02) && (dev->int_status & SERIAL_INT_TRANSMIT)) {
        /* Transmit data empty */
        stat     = 1;
        dev->iir = 0x02;
    } else if ((dev->ier & 0x08) && (dev->int_status & SERIAL_INT_MSR)) {
        /* Modem status interrupt */
        stat     = 1;
        dev->iir = 0x00;
    } else if ((dev->ier & 0x40) && (dev->int_status & SERIAL_INT_RX_DMA_TC)) {
        /* DMA Reception End of Transfer interrupt */
        stat     = 1;
        dev->iir = 0x0e;
    } else if ((dev->ier & 0x80) && (dev->int_status & SERIAL_INT_TX_DMA_TC)) {
        /* DMA Transmission End of Transfer interrupt */
        stat     = 1;
        dev->iir = 0x0a;
    }
...
{
    if (dev->mctrl & 0x10) {
        // pclog("Write to loopback: %02X\n", val);
        write_fifo(dev, val);
    } else if (dev->sd->dev_write) {
        // pclog("Write to device  : %02X\n", val);
        dev->sd->dev_write(dev, dev->sd->priv, val);
    } // else
        // pclog("Write to nowhere : %02X\n", val);
...
{
    dev->lsr           = (dev->lsr & 0x9f) | 0x60;
    dev->int_status    = (dev->int_status & ~SERIAL_INT_TRANSMIT) | SERIAL_INT_TRANSMIT;
    dev->xmit_fifo_pos = 0;
...
serial_reset(void *priv)
{
    ...
    if (com_ports[dev->inst].enabled) {
        ...
        dev->xmit_fifo_pos = dev->xmit_fifo_end = 0x00;
