...
// Note: The entire raw sector data is encoded in the 'img' file, and subchannels are encoded in 'sub' file.
// TODO: Do multi-session images actually store the space between multi-sessions in the image?
// Note 2: Data tracks are not supposed to have pregaps, only audio tracks, and those are fixed to 150 sectors too?
...
ccd_image_open(cdrom_t *dev, const char *path)
{
    ...
    img->sub_file = fopen(img_path, "rb");
    if (!img->sub_file) {
        img_path[strlen(img_path) - 1] = 'b';
        img_path[strlen(img_path) - 2] = 'u';
