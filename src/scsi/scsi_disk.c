 ...
 *
 * Version:	@(#)scsi_disk.c	1.0.19	2018/03/26
 *
...
find_hdd_for_scsi_id(uint8_t scsi_id, uint8_t scsi_lun)
{
	...
	if ((hdd[i].spt == 0) || (hdd[i].hpc == 0) || (hdd[i].tracks == 0))
		continue;
	if ((hdd[i].bus == HDD_BUS_SCSI) && (hdd[i].scsi_id == scsi_id) && (hdd[i].scsi_lun == scsi_lun))
		return i;
    }
    return 0xff;
}
...
build_scsi_hd_map(void)
{
	...
	for (j = 0; j < 8; j++) {
		...
		if (scsi_hard_disks[i][j] != 0xff) {
			memset(&(shdc[scsi_hard_disks[i][j]]), 0,
			       sizeof(shdc[scsi_hard_disks[i][j]]));
...
scsi_hd_data_command_finish(uint8_t id, int len, int block_len, int alloc_len, int direction)
{
    scsi_hd_log("SCSI HD %i: Finishing command (%02X): %i, %i, %i, %i, %i\n", id,
		shdc[id].current_cdb[0], len, block_len, alloc_len, direction, shdc[id].request_length);
    if (alloc_len >= 0) {
	if (alloc_len < len)
		len = alloc_len;
    }
...
scsi_hd_command(uint8_t id, uint8_t *cdb)
{
	...
	scsi_hd_log("SCSI HD %i: CDB: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n", id,
	...
	case GPCMD_READ_12:
		...
		if ((shdc[id].sector_pos > last_sector) ||
		    ((shdc[id].sector_pos + shdc[id].sector_len - 1) > last_sector)) {
