...
static uint8_t convertible_diag_value;
static uint8_t convertible_diag_enabled;
static uint8_t convertible_diag_lines[8];
static void (*convertible_wake)(void);
...
{
    return !(dev->flags & PIC_CONVERTIBLE) && !(dev->icw1 & 2);
}
...
find_best_interrupt(pic_t *dev)
{
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        if ((dev->isr & b) &&
            !((dev->flags & PIC_CONVERTIBLE) && dev->special_mask_mode && (dev->imr & b)))
            break;
...
pic_update_pending_xt(void)
{
    ...
    if (pic.int_pending && (pic.flags & PIC_CONVERTIBLE) && convertible_wake)
        convertible_wake();
}
...
pic_reset(void)
{
    ...
    const uint8_t board_flags = pic.flags & PIC_CONVERTIBLE;
...
pic_action(pic_t *dev, uint8_t irq, uint8_t eoi, uint8_t rotate)
{
    ...
    if (irq != 0xff) {
        if (eoi)
            dev->isr &= ~b;
        if (rotate && !(dev->flags & PIC_CONVERTIBLE))
...
#endif
        if ((dev->ocw3 & 0x04) && (!(dev->flags & PIC_CONVERTIBLE) || !(addr & 1))) {
            dev->interrupt &= ~0x20; /* Freeze the interrupt until the poll is over. */
            if (dev->flags & PIC_CONVERTIBLE) {
            ...
            if (dev->int_pending) {
                ...
                dev->data_bus = (dev->flags & PIC_CONVERTIBLE) ? (dev->interrupt & 7) : 0x00;
            dev->ocw3 &= ~0x04;
        } else if (addr & 0x0001)
...
pic_write(uint16_t addr, uint8_t val, void *priv)
{
    ...
    if (addr & 0x0001) {
        switch (dev->state) {
            ...
            case STATE_NONE:
                dev->imr = val;
                if ((dev->flags & PIC_CONVERTIBLE) || (is286 && !force_xt_imr_timing) || dev->zenith)
                    update_pending();
        ...
        if (val & 0x10) {
            ...
            for (uint8_t i = 0; i <= 7; i++) {
                if (dev->lines[i] > 0 &&
                    (!(dev->flags & PIC_CONVERTIBLE) || pic_level_triggered(dev, i)))
                    dev->irr              |= (1 << i);
            }
            ...
            dev->ocw3 = ((dev->flags & PIC_CONVERTIBLE) && !(val & 2))
...
pic_reset_hard(void)
{
    ...
    pic.flags &= ~PIC_CONVERTIBLE;
    convertible_wake = NULL;
    pic_reset();
...
void
pic_init_convertible(void (*wake)(void))
{
    pic.flags |= PIC_CONVERTIBLE;
    convertible_wake = wake;
    convertible_diag_value = convertible_diag_enabled = 0;
    memset(convertible_diag_lines, 0, sizeof(convertible_diag_lines));
}

uint8_t
pic_convertible_diag_read(uint8_t selector)
{
    if (!(selector & 0x40))
...
static void
pic_convertible_diag_update(void)
{
    ...
    const uint8_t requests = (convertible_diag_value & 0xfc) |
                             ((convertible_diag_value & 2) >> 1);
    for (unsigned i = 0; i < 8; i++) {
        const int asserted = convertible_diag_enabled && (requests & (1 << i));
        if (!!asserted != convertible_diag_lines[i])
            picint_common(1 << i, PIC_IRQ_LEVEL, !!asserted, &convertible_diag_lines[i]);
    }
}

void
pic_convertible_diag_write(uint8_t value)
{
    convertible_diag_value = value;
    pic_convertible_diag_update();
}

void
pic_convertible_diag_enable(int enabled)
{
    convertible_diag_enabled = !!enabled;
    pic_convertible_diag_update();
}
...
{
    return (dev->flags & PIC_CONVERTIBLE) || (dev->icw4 & 1);
}
