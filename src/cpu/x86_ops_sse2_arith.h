...
static int opPDADBW_mm_mm_a16(uint32_t fetchdat)
{
    MMX_ENTER();
    fetch_ea_16(fetchdat);
    if (cpu_mod == 3)
...
static int opPDADBW_mm_mm_a32(uint32_t fetchdat)
{
    MMX_ENTER();
    fetch_ea_32(fetchdat);
    if (cpu_mod == 3)
...
static int opPDADBW_xmm_xmm_a16(uint32_t fetchdat)
{
    fetch_ea_16(fetchdat);
    if (cpu_mod == 3)
    {
...
static int opPDADBW_xmm_xmm_a32(uint32_t fetchdat)
{
    fetch_ea_32(fetchdat);
    if (cpu_mod == 3)
    {
...
static int opPDADBW_a16(uint32_t fetchdat)
{
    if(sse_xmm) return opPDADBW_xmm_xmm_a16(fetchdat);
    else return opPMULHUW_mm_mm_a16(fetchdat);
}

static int opPDADBW_a32(uint32_t fetchdat)
{
    if(sse_xmm) return opPDADBW_xmm_xmm_a32(fetchdat);
    else return opPDADBW_mm_mm_a32(fetchdat);
}