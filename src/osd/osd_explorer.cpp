...
const char *entry_label(const OsdExplorer::Entry &entry)
{
    switch (entry.kind) {
        case OsdExplorer::EntryKind::Parent:
            return "..";
        default:
...
void
OsdExplorer::Open(const OsdExplorerConfig &config)
{
    config_               = config;
    is_open_              = true;
...
OsdExplorer::Draw()
{
    ...
    const bool has_show_all_files_checkbox = config_.mode == OsdExplorerMode::File && HasVisibleFilter();
    auto preferred_list_slot = [&]() {
        if constexpr (!kUseSplitPanes)
            return FocusSlot::EntryList;

        if (config_.mode == OsdExplorerMode::File)
    ...
    auto slot_is_enabled = [&](FocusSlot slot) {
        switch (slot) {
            ...
            case FocusSlot::FileList:
                return kUseSplitPanes && config_.mode == OsdExplorerMode::File;
    ...
    if constexpr (kUseSplitPanes) {
        ...
        ImGui::BeginChild("##directories", ImVec2(config_.mode == OsdExplorerMode::Directory ? 0 : pane_width, -button_row_height), true);
    ...
    if (!filename_input_active && enter) {
        if (focused_slot_ == FocusSlot::Accept && can_accept) {
            ...
            result.type = OsdExplorerResultType::Accepted;
            is_open_    = false;
            ImGui::End();
            return result;
        }
        if (focused_slot_ == FocusSlot::Cancel) {
            result.type = OsdExplorerResultType::Cancelled;
            is_open_    = false;
            ImGui::End();
            return result;
        }
...
void
OsdExplorer::QueueFocusSlot(FocusSlot slot)
{
...
OsdExplorer::TryHandleFilenameInput(OsdExplorerResult *result)
{
    ...
    if (config_.mode == OsdExplorerMode::File && is_file) {
...
bool
OsdExplorer::TryActivateEntry(const Entry &entry, OsdExplorerResult *result)
{
    switch (entry.kind) {
...
OsdExplorer::BuildAcceptedPath(fs::path *path) const
{
    ...
    if (ResolveTypedPath(&resolved_path, &is_directory, &is_file)) {
        if (config_.mode == OsdExplorerMode::Directory && is_directory) {
            *path = resolved_path;
            return true;
        }
        if (config_.mode == OsdExplorerMode::File && is_file) {
...
bool
OsdExplorer::HasVisibleFilter() const
{
    return config_.extension_globs != nullptr;
}

const OsdExplorer::Entry *
OsdExplorer::SelectedDirectoryEntry() const
{
...
const OsdExplorer::Entry *
OsdExplorer::SelectedFileEntry() const
{
...
const OsdExplorer::Entry *
OsdExplorer::SelectedVisibleEntry() const
{
