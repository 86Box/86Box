...
int
machine_at_65kv2_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear(L"roms/machines/65kv2/65kv213.bin",
			   0x000c0000, 262144, 0);
    ...
    hwm_values_t machine_hwm = {
    	{    /* fan speeds */
    		...
    		3300,				   /* +3.3V */
    		RESISTOR_DIVIDER(5000,   11,  16), /* +5V  (divider values bruteforced) */
    		RESISTOR_DIVIDER(12000,  28,  10), /* +12V (divider values bruteforced) */
    		RESISTOR_DIVIDER(12000,  59,  20), /* -12V (divider values bruteforced) */
    		RESISTOR_DIVIDER(5000,    1,   2)  /* -5V  (divider values bruteforced) */
    	}
    };
...
int
machine_at_6vx7_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear(L"roms/machines/6vx7/6VX7-4X.F45",
			   0x000c0000, 262144, 0);
