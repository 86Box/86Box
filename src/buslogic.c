...
static void BuslogicCommandComplete(Buslogic_t *Buslogic)
{
	...
	if (Buslogic->Command != CMD_START_SCSI)
	{
...
{
	uint8_t Id = BuslogicRequests->TargetID;	
	...
	{		
			...
			CmdBlock->common.Opcode == SCATTER_GATHER_COMMAND_RES)
		{
			...
			SCSIDevices[Id].buffer_size = DataToTransfer;
			...
			if (CmdBlock->common.ControlByte == CCB_DATA_XFER_OUT || CmdBlock->common.ControlByte == 0x00)
			{
				...
				do
				{
					...
					for (ScatterEntry = 0; ScatterEntry < ScatterGatherRead; ScatterEntry++)
					{
						...
						DMAPageRead(Address, SCSIDevices[Id].CmdBuffer, DataToTransfer);
					}
				...
				CmdBlock->common.Opcode == SCSI_INITIATOR_COMMAND_RES)
		{
			...
			SCSIDevices[Id].InitLength = DataLength;
			
			DMAPageRead(Address, SCSIDevices[Id].CmdBuffer, SCSIDevices[Id].InitLength);
		}
	}
}
...
void BuslogicDataBufferFree(BuslogicRequests_t *BuslogicRequests)
{
	...
	{	
			...
			BuslogicRequests->CmdBlock.common.Opcode == SCATTER_GATHER_COMMAND_RES)
		{
			...
			do
			{
				...
				for (ScatterEntry = 0; ScatterEntry < ScatterGatherRead; ScatterEntry++)
				{
					...
					DMAPageWrite(Address, SCSIDevices[Id].CmdBuffer, DataToTransfer);
				}
		...
		{
			uint32_t Address = DataPointer;
			DMAPageWrite(Address, SCSIDevices[Id].CmdBuffer, SCSIDevices[Id].InitLength);
		}
	}
}
...
static void BuslogicSenseBufferAllocate(BuslogicRequests_t *BuslogicRequests)
{
	...
	if (SenseLength)
		BuslogicRequests->RequestSenseBuffer = malloc(SenseLength);
}
...
{	
	...
	{	
		...
		{	
			...
			{
				SCSIRead(Id, SCSIDevices[Id].buffer_size);
			}
			else if (CmdBlock.common.ControlByte == CCB_DATA_XFER_OUT)
			{
				SCSIWrite(Id, SCSIDevices[Id].buffer_size);
			}
			...
			{
				//If the executed command doesn't return any length, end the request immediately.
				SCSIDevices[Id].buffer_size = 0;
			}
			...
			if (SCSIDevices[Id].InitLength >= SCSIDevices[Id].buffer_size)
