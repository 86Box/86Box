...
svga_poll(void *p)
{
    ...
    if (!vga_on && ibm8514_enabled && ibm8514_on && (dev->local < 2)) {
    ...
    if (!svga->linepos) {
        ...
        if (svga->interlace)
            svga->displine++;
        if ((svga->cgastat & 8) && ((svga->displine & 0x0f) == (svga->crtc[0x11] & 0x0f)) && svga->vslines)
            svga->cgastat &= ~8;
        svga->vslines++;
        ...
        svga->linepos = 0;
        if ((svga->sc == (svga->crtc[0x0b] & 31)) || (svga->sc == svga->rowcount))
            svga->con = 0;
        if (svga->dispon) {
