...
static uint8_t s3_virge_in(uint16_t addr, void *p)
{
		...
		break;
                ...
                switch (svga->crtcreg)
                {
                        ...
                        case 0x45: virge->hwc_col_stack_pos = 0; break;
			...
			case 0x6b: return svga->crtc[0x59] & 0xfc;
			case 0x6c: return 0;
                }
...
static void s3_virge_updatemapping(virge_t *virge)
{
        ...
        if (virge->pci && !(virge->pci_regs[PCI_REG_COMMAND] & PCI_COMMAND_MEM))
        {
...
s3_virge_mmio_read(uint32_t addr, void *p)
{
		...
		ret |= virge->subsys_stat;
                ...
                ret = s3_virge_in(addr & 0x3ff, virge);
		break;
        }
        return ret;
}
...
s3_virge_mmio_read_w(uint32_t addr, void *p)
{
	...
	if (addr & 0x8000)
		ret = s3_virge_mmio_read(addr & 0xfffe, virge) |
			(s3_virge_mmio_read((addr & 0xfffe) + 1, virge) << 8);
...
s3_virge_mmio_read_l(uint32_t addr, void *p)
{
	...
	if (addr & 0x8000) {
		switch (addr & 0xfffc) {
			...
			default:
			ret = s3_virge_mmio_read(addr & 0xfffc, virge) |
				(s3_virge_mmio_read((addr & 0xfffc) + 1, virge) << 8) | 
				(s3_virge_mmio_read((addr & 0xfffc) + 2, virge) << 16) |
				(s3_virge_mmio_read((addr & 0xfffc) + 3, virge) << 24);
			break;
		}
	} else if ((addr & 0xfffc) >= 0xa000) {
		switch (addr & 0xfffc) {
			case 0xa4d4:
...
static void
s3_virge_write_fifo(virge_t *virge, uint32_t addr, uint8_t val)
{
	if ((addr & 0xffff) < 0x8000) {
		...
		switch (addr & 0xffff) {
			...
			case 0x83dc: case 0x83dd: case 0x83de: case 0x83df:
				...
				break;
			case 0x8504:
				virge->subsys_stat &= ~val;
				s3_virge_update_irqs(virge);
				break;
			case 0x8505:
				virge->subsys_cntl = val;
				s3_virge_update_irqs(virge);
				break;
			case 0x850c:
				virge->advfunc_cntl = val;
				s3_virge_updatemapping(virge);
				break;
		}	
	}
...
static void
s3_virge_write_fifo_w(virge_t *virge, uint32_t addr, uint16_t val)
{
	if ((addr & 0xfffe) < 0x8000) {
		if (virge->s3d.cmd_set & CMD_SET_MS) {
			...
			s3_virge_bitblt(virge, 16, val);
                ...
                s3_virge_mmio_write(addr & 0xfffe, val, virge);
                s3_virge_mmio_write((addr & 0xfffe) + 1, val >> 8, virge);
	}
}

static void
s3_virge_write_fifo_l(virge_t *virge, uint32_t addr, uint32_t val)
{
	svga_t *svga = &virge->svga;
	
	if ((addr & 0xfffc) >= 0x8000 && (addr & 0xfffc) < 0xa000) {
		switch (addr & 0xfffc) {		
			case 0x8180:
			...
			default:
			s3_virge_mmio_write(addr & 0xfffc, val, virge);
			s3_virge_mmio_write((addr & 0xfffc) + 1, val >> 8, virge);
			s3_virge_mmio_write((addr & 0xfffc) + 2, val >> 16, virge);
			s3_virge_mmio_write((addr & 0xfffc) + 3, val >> 24, virge);
			break;
		}
	} else if ((addr & 0xfffc) < 0x8000) {
		if (virge->s3d.cmd_set & CMD_SET_MS) {
...
static void fifo_thread(void *param)
{
        ...
        while (1)
        {
				...
				s3_virge_write_fifo(virge, fifo->addr_type & FIFO_ADDR, fifo->val);
				break;
                                case FIFO_WRITE_WORD:
                                s3_virge_write_fifo_w(virge, fifo->addr_type & FIFO_ADDR, fifo->val);
                                break;
                                case FIFO_WRITE_DWORD:
				s3_virge_write_fifo_l(virge, fifo->addr_type & FIFO_ADDR, fifo->val);
                                break;
                        }
...
{
        virge_t *virge = (virge_t *)p;
        s3_virge_queue(virge, addr & 0xffff, val, FIFO_WRITE_BYTE);
}
static void
s3_virge_mmio_write_w(uint32_t addr, uint16_t val, void *p)
{
        virge_t *virge = (virge_t *)p;
	s3_virge_queue(virge, addr & 0xffff, val, FIFO_WRITE_WORD);
}
static void
s3_virge_mmio_write_l(uint32_t addr, uint32_t val, void *p)
{
        virge_t *virge = (virge_t *)p;
	s3_virge_queue(virge, addr & 0xffff, val, FIFO_WRITE_DWORD);
}
...
static void s3_virge_bitblt(virge_t *virge, int count, uint32_t cpu_dat)
{
	svga_t *svga = &virge->svga;
        ...
        int x_mul = -1;
        int cpu_dat_shift = 0;
        uint32_t *pattern_data = NULL;
        uint32_t src_fg_clr = 0, src_bg_clr = 0;
	uint32_t src_addr;
	uint32_t dest_addr;
	...
	int update;
        ...
        switch (virge->s3d.cmd_set & CMD_SET_COMMAND_MASK)
        {
				...
				virge->s3d.src_y &= 0x7ff;
                                virge->s3d.dest_y += y_inc;
				virge->s3d.dest_y &= 0x7ff;
                                virge->s3d.h--;
...
static void s3_virge_triangle(virge_t *virge, s3d_t *s3d_tri)
{
        ...
        switch (((s3d_tri->cmd_set >> 12) & 7) | ((s3d_tri->cmd_set & (1 << 29)) ? 8 : 0))
        {
                ...
                break;
                case (0 | 8): case (1 | 8):
                if (virge->chip == S3_VIRGEDX || virge->chip == S3_TRIO3D2X)
...
static uint8_t s3_virge_pci_read(int func, int addr, void *p)
{
		...
		return virge->pci_regs[PCI_REG_COMMAND]; /*Respond to IO and memory accesses*/
                ...
                case 0x0d: return virge->pci_regs[0x0d] & 0xf8;
                ...
                case 0x30: return virge->pci_regs[0x30] & 0x01; /*BIOS ROM address*/
                case 0x31: return 0x00;
                case 0x32: return virge->pci_regs[0x32];
                case 0x33: return virge->pci_regs[0x33];

                case 0x3c: return virge->int_line;
                case 0x3d: return PCI_INTA;
                
                case 0x3e: return 0x04;
                case 0x3f: return 0xff;
...
static void *s3_virge_init(const device_t *info)
{
	...
	virge->int_line = 0;
        ...
        switch (virge->memory_size)
        {
			...
			virge->svga.crtc[0x36] = 1 | (0 << 2) | (1 << 4) | (4 << 5);
                break;
                case 4:
                default:
		if (virge->pci)	
			...
			virge->svga.crtc[0x36] = 1 | (0 << 2) | (1 << 4) | (0 << 5);
                break;
		case 8:
        ...
        virge->svga.crtc[0x59] = virge->pci ? 0x70 : 0x0a;
...
static int s3_trio3d_2x_available(void)
{
        return rom_present(ROM_TRIO3D_2X);
}
...
const device_t s3_trio3d_2x_pci_device =
{
        "S3 Trio3D/2X PCI",
        DEVICE_PCI,
        S3_TRIO3D_2X,
