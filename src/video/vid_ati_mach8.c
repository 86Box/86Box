...
#define BIOS_MACH32_VLB_ROM_PATH        "roms/video/mach32/ATI Mach32 (VLB) (113-19509-102) (1993-08-27) (27256).BIN"
#define BIOS_MACH32_MCA_ROM_PATH        "roms/video/mach32/MACH32MCA.BIN"
...
typedef struct mach_t {
    ...
    uint16_t cursor_col_0_rg, cursor_col_1_rg;
...
static uint8_t  mach_in(uint16_t addr, void *p);
...
#define READ(addr, dat) \
        if (!dev->bpp) \
            dat = (dev->local >= 2) ? svga->vram[(addr) & (svga->vram_mask)] :  dev->vram[(addr) & (dev->vram_mask)]; \
        else \
            ...
            if (!cpu_input) {
                ...
                if (mach->accel.sx_end > mach->accel.sx_start) {
                    ...
                    if ((mach->accel.dp_config == 0x2071) || (mach->accel.dp_config == 0x2011)) {
                        mach_log("BitBLT: Src Positive X: wh(%d,%d), srcwidth = %d, coordinates: %d,%d px, start: %d, end: %d px, stepx = %d, dpconfig = %04x, fgmix = %x, bkgdmix = %x.\n", mach->accel.width, mach->accel.height, mach->accel.src_width, dev->accel.cx, dev->accel.cy, mach->accel.src_x_start, mach->accel.src_x_end, mach->accel.src_stepx, mach->accel.dp_config, dev->accel.frgd_mix & 0x1f, dev->accel.bkgd_mix & 0x1f);
                    }
                ...
                if ((svga->bpp == 24) && (frgd_sel == 5)) {
                    ...
                    mach_log("BitBLT=%04x, Pitch=%d, C(%d,%d), SRCWidth=%d, WH(%d,%d), clr(%d,%d).\n", mach->accel.dp_config, dev->ext_pitch, dev->accel.cx, dev->accel.cy, mach->accel.src_width, mach->accel.width, mach->accel.height, clip_l, clip_r);
    ...
    switch (addr) {
        ...
        case 0x1cf:
            switch (mach->index) {
                ...
                case 0xb7:
                    temp = 0;
                    temp |= ((ati_eeprom_read(&mach->eeprom)) << 3);
                    mach_log("EEPROM read = %02x.\n", temp & 8);
                    break;
        ...
        case 0x3C9:
            ...
            if (dev->local >= 2) {
                if ((mach->pci_bus) && !mach->ramdac_type)
                    return ati68860_ramdac_in((addr & 3) | (rs2 << 2) | (rs3 << 3), svga->ramdac, svga);
                else
                    return ati68875_ramdac_in(addr, rs2, rs3, svga->ramdac, svga);
            }
...
mach_recalctimings(svga_t *svga)
{
    ...
    if (!svga->scrblank && (svga->crtc[0x17] & 0x80) && svga->attr_palette_enable) {
        if (((svga->gdcreg[6] & 1) || (svga->attrregs[0x10] & 1)) && vga_on) {
                ...
                switch (svga->bpp) {
                    ...
                    case 24:
                        if (mach->accel.ext_ge_config & 0x400)
                            svga->render = svga_render_BGR_highres;
                        else
        ...
        case 0xfee8:
            if (len == 1)
                ...
                if ((dev->accel.multifunc_cntl >> 12) == 5) {
                    if (!dev->ext_crt_pitch || ((dev->local < 2) && dev->ext_crt_pitch))
...
mach_accel_out(uint16_t port, uint8_t val, mach_t *mach)
{
    ...
    switch (port) {
        ...
        case 0x7eef:
            ...
            if (mach->accel.eeprom_control & 8)
                ati_eeprom_write(&mach->eeprom, mach->accel.eeprom_control & 4, mach->accel.eeprom_control & 2, mach->accel.eeprom_control & 1);
            ibm8514_on = !!(mach->accel.eeprom_control & 0x1000);
            vga_on = !ibm8514_on;
            mach_log("7EEE VGA ON = %d, Ext = %i, val = %04x.\n", vga_on, ibm8514_on, mach->accel.eeprom_control & 0xff);
            svga_recalctimings(svga);
            break;
    }
}
...
mach_accel_in_fifo(mach_t *mach, svga_t *svga, ibm8514_t *dev, uint16_t port, int len)
{
    ...
    switch (port) {
        ...
        case 0x92ee:
            if (len == 1)
                temp = mach->accel.eeprom_control & 0xf0;
            else
                temp = mach->accel.eeprom_control & 0xfff0;
            break;
        case 0x92ef:
            if (len == 1)
...
mach32_ap_writew(uint32_t addr, uint16_t val, void *p)
{
    ...
    if (((addr >= ((mach->ap_size << 20) - 0x200)) && (addr < (mach->ap_size << 20)))) {
        ...
        mach_log("Linear WORDW Write=%08x, val=%04x, fast=%d.\n", addr, val, mach->svga.fast);
...
mach32_ap_writel(uint32_t addr, uint32_t val, void *p)
{
    ...
    if (((addr >= ((mach->ap_size << 20) - 0x200)) && (addr < (mach->ap_size << 20)))) {
        ...
        mach_log("Linear WORDL Write=%08x, val=%08x, fast=%d.\n", addr, val, mach->svga.fast);
    ...
    for (int x = 0; x < 64; x += 8) {
        ...
        for (int xx = 0; xx < 8; xx++) {
            ...
            if (offset >= svga->hwcursor_latch.x) {
                switch (comb) {
                    case 0:
                        buffer32->line[displine][offset + svga->x_add] = color0;
                        break;
                    case 1:
                        buffer32->line[displine][offset + svga->x_add] = color1;
                        break;
                    case 3:
...
static void
mach_mca_write(int port, uint8_t val, void *p)
{
    mach_t *mach = (mach_t *) p;
    ...
    //    mem_mapping_set_addr(&mach->bios_rom.mapping, (((mach->pos_regs[3] & 0x3e) << 0x0c) >> 1) + 0xc0000, 0x1000);
    //}
}
...
mach8_init(const device_t *info)
{
    ...
    if (dev->local >= 2) {
        ...
        if (mach->vlb_bus)
            ...
            if (mach->has_bios)
                mem_mapping_disable(&mach->bios_rom.mapping);
            pci_add_card(PCI_ADD_VIDEO, mach32_pci_read, mach32_pci_write, mach);
...
const device_t mach32_pci_onboard_device = {
