                    ...
                    case S3_VISION968:
                        switch (s3->ramdac_type) {
                            ...
                            case TVP3026: /*TVP3026 RAMDAC and clock chip*/
                                ...
                                if ((clk_sel == 2) || ((clk_sel == 3) && (s3->width >= 1024))) {
                                    svga->hdisp <<= 1;
                                    svga->dots_per_clock <<= 1;
                                    svga->clock *= 2.0;
                                }
                    ...
                    if (((s3->accel.multifunc[0xa] & 0xc0) == 0x80) || (s3->accel.cmd & 0x02)) {
                        ...
                        if (s3->accel.rd_mask_16bit_check) {
                            if (s3->accel.cmd == 0x53b0) {
                                const uint16_t temp1 = vram_w[dword_remap_w(svga, s3->accel.dest + s3->accel.cx - s3->accel.minus) & (s3->vram_mask >> 1)];
                                const uint16_t temp2 = vram_w[dword_remap_w(svga, s3->accel.dest + s3->accel.cx - s3->accel.minus + 1) & (s3->vram_mask >> 1)];
                                if (s3->accel.cur_x & 0x400) {
                                    temp = temp1 >> 8;
                                    temp |= (temp2 >> 8) << 8;
                                } else {
        ...
        case 2: /*Rectangle fill*/
            ...
            if ((s3->bpp == 2) || (svga->bpp == 24)) {
                ...
                while (count-- && (s3->accel.sy >= 0)) {
                    ...
                    if (s3->accel.cmd & 0x20)
                        s3->accel.cx = (int16_t) (s3->accel.cx + multiplier);
                    else
                        s3->accel.cx = (int16_t) (s3->accel.cx - multiplier);
                    ...
                    if (s3->accel.sx < 0) {
                        ...
                        if (s3->accel.cmd & 0x80)
                            s3->accel.cy = (int16_t) (s3->accel.cy + multiplier);
                        else
                            s3->accel.cy = (int16_t) (s3->accel.cy - multiplier);
        ...
        case 6: /*BitBlt*/
            if (!cpu_input) { /*!cpu_input is trigger to start operation*/
                ...
                if (s3->bpp == 2) {
                    ...
                    s3->accel.blit_24bpp = (int16_t) ((s3->accel.blit_24bpp + 1) & 3);
                } else
                    s3->accel.blit_24bpp = 0;
            ...
            if ((s3->bpp == 2) || (svga->bpp == 24)) {
                ...
                if (!cpu_input && (frgd_mix == 3) && !vram_mask && !(s3->accel.multifunc[0xe] & 0x100) && ((s3->accel.cmd & 0xa0) == 0xa0) && ((s3->accel.frgd_mix & 0xf) == 7) && ((s3->accel.bkgd_mix & 0xf) == 7)) {
                    ...
                    while (1) {
                        ...
                        s3->accel.cx = (int16_t) (s3->accel.cx + multiplier);
                        s3->accel.dx = (int16_t) (s3->accel.dx + multiplier);
                        s3->accel.sx--;
                        ...
                        if (s3->accel.sx < 0) {
                            ...
                            s3->accel.cy = (int16_t) (s3->accel.cy + multiplier);
                            s3->accel.dy = (int16_t) (s3->accel.dy + multiplier);
                            ...
                            if (s3->accel.sy < 0) {
                                s3->accel.destx_distp = (int16_t) (s3->accel.dx / multiplier);
                                s3->accel.desty_axstp = (int16_t) (s3->accel.dy / multiplier);
                                return;
                            }
                        }
                    }
        ...
        case 7: /*Pattern fill - BitBlt but with source limited to 8x8*/
            ...
            if (s3->bpp == 2) {
                ...
                while (count-- && (s3->accel.sy >= 0)) {
                    ...
                    if (s3->accel.cmd & 0x20) {
                        s3->accel.cx = (int16_t) (((s3->accel.cx + 3) & 7) | (s3->accel.cx & ~7));
                        s3->accel.dx += 3;
                    } else {
                        s3->accel.cx = (int16_t) (((s3->accel.cx - 3) & 7) | (s3->accel.cx & ~7));
                        s3->accel.dx -= 3;
                    ...
                    if (s3->accel.sx < 0) {
                        ...
                        if (s3->accel.cmd & 0x80) {
                            s3->accel.cy = (int16_t) (((s3->accel.cy + 3) & 7) | (s3->accel.cy & ~7));
                            s3->accel.dy += 3;
                        } else {
                            s3->accel.cy = (int16_t) (((s3->accel.cy - 3) & 7) | (s3->accel.cy & ~7));
                            s3->accel.dy -= 3;
                        ...
                        if (s3->accel.sy < 0) {
                            s3->accel.destx_distp = (int16_t) (s3->accel.dx / 3);
                            s3->accel.desty_axstp = (int16_t) (s3->accel.dy / 3);
                            return;
                        }
                    }
                }
