...
static uint16_t
s3_accel_in_pixtrans_w(s3_t *s3, uint32_t port)
{
	svga_t *svga = &s3->svga;
...
static uint32_t
s3_accel_in_pixtrans_l(s3_t *s3, uint32_t port)
{
	if ((s3->accel.multifunc[0xa] & 0xc0) == 0x80) {	
		if ((s3->accel.cmd & 0x600) == 0x400) {	
			s3_accel_start(32, 1, 0xffffffff, 0, s3);	
...
s3_accel_in(uint16_t port, void *p)
{
	...
	switch (port) {
		...
		if ((s3->accel.multifunc[0xa] & 0xc0) == 0x80 && !(s3->accel.cmd & 0x600) && s3_cpu_dest(s3) &&
			(((s3->accel.frgd_mix & 0x60) != 0x40) || ((s3->accel.bkgd_mix & 0x60) != 0x40)))
			s3_accel_start(8, 1, 0xffffffff, 0, s3);
		else if (!(s3->accel.cmd & 0x600) && s3_cpu_dest(s3))
...
s3_accel_in_w(uint16_t port, void *p)
{
    ...
    if (s3_cpu_dest(s3))
	temp = s3_accel_in_pixtrans_w(s3, port);
...
s3_accel_in_l(uint16_t port, void *p)
{
    ...
    if (s3_cpu_dest(s3))
	temp = s3_accel_in_pixtrans_l(s3, port);
...
s3_accel_read_w(uint32_t addr, void *p)
{
	...
	if (addr & 0x8000) {
	    ...
	    temp = s3_accel_in_pixtrans_w(s3, addr & 0xfffe);
    }
...
s3_accel_read_l(uint32_t addr, void *p)
{
	...
	if (addr & 0x8000) {
	    ...
	    temp = s3_accel_in_pixtrans_l(s3, addr & 0xfffc);    
    }
