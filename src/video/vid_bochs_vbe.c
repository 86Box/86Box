...
static void gen_mode_info(int hdisplay, int vdisplay, float vrefresh,
...
#define CVT_C_PRIME                                                            \
  ...
  int hblank =
      mode_info->hdisplay * hblank_percentage / (100.0 - hblank_percentage);
...
void bochs_vbe_recalctimings(svga_t *svga) {
  ...
  if (dev->vbe_regs[VBE_DISPI_INDEX_ENABLE] & VBE_DISPI_ENABLED) {
    ...
    if (dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH] <
        dev->vbe_regs[VBE_DISPI_INDEX_XRES])
    ...
    if (svga->bpp == 4) {
      ...
      svga->memaddr_latch =
          (dev->vbe_regs[VBE_DISPI_INDEX_Y_OFFSET] * svga->rowoffset) +
          (dev->vbe_regs[VBE_DISPI_INDEX_X_OFFSET] >> 3);
      svga->fullchange = 3;
    } else {
      svga->rowoffset = dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH] *
                        ((svga->bpp == 15) ? 2 : (svga->bpp / 8));
      svga->memaddr_latch =
          (dev->vbe_regs[VBE_DISPI_INDEX_Y_OFFSET] * svga->rowoffset) +
          (dev->vbe_regs[VBE_DISPI_INDEX_X_OFFSET] *
           ((svga->bpp == 15) ? 2 : (svga->bpp / 8)));
      svga->fullchange = 3;
    }
    if (svga->bpp == 4)
      dev->vbe_regs[VBE_DISPI_INDEX_VIRT_HEIGHT] =
          (svga->vram_max * 2) / dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH];
    else
      dev->vbe_regs[VBE_DISPI_INDEX_VIRT_HEIGHT] =
          (svga->vram_max / ((svga->bpp == 15) ? 2 : (svga->bpp / 8))) /
          dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH];
    svga->split = 0xffffff;
    switch (svga->bpp) {
    case 4:
...
uint16_t bochs_vbe_inw(uint16_t addr, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  uint32_t val;
...
uint32_t bochs_vbe_inl(uint16_t addr, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  uint32_t ret;
  if (addr == 0x1ce)
    ret = dev->vbe_index;
...
static void recalculate_data(void *priv) {
   ...
   * big VRAM. */
  dev->vbe_regs[VBE_DISPI_INDEX_VIRT_HEIGHT] =
      cVirtHeight >= UINT16_MAX ? UINT16_MAX : (uint16_t)cVirtHeight;
}
void bochs_vbe_outw(uint16_t addr, uint16_t val, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  svga_t *svga = &dev->svga;
  uint32_t max_bank;
  if (addr == 0x1ce) {
    ...
    if (dev->vbe_index <= VBE_DISPI_INDEX_NB) {
      ...
      case VBE_DISPI_INDEX_ID:
        if (val == VBE_DISPI_ID0 || val == VBE_DISPI_ID1 ||
            ...
            val == VBE_DISPI_ID4 || val == VBE_DISPI_ID5 ||
            /* VBox extensions. */
      ...
      case VBE_DISPI_INDEX_BPP:
        if (val == 0)
          val = 8;
        if (val == 4 || val == 8 || val == 15 || val == 16 || val == 24 ||
            val == 32) {
          dev->vbe_regs[dev->vbe_index] = val;
          fRecalculate = true;
        }
      ...
      case VBE_DISPI_INDEX_BANK:
        if (dev->vbe_regs[VBE_DISPI_INDEX_BPP] <= 4)
          max_bank = ((dev->vram_size >> 16) - 1) >>
                     2; /* Each bank really covers 256K */
      ...
      case VBE_DISPI_INDEX_ENABLE: {
        if ((val & VBE_DISPI_ENABLED) &&
          ...
          if (dev->vbe_regs[VBE_DISPI_INDEX_BPP] == 4)
            ...
            cb = dev->vbe_regs[VBE_DISPI_INDEX_XRES] *
                 ((dev->vbe_regs[VBE_DISPI_INDEX_BPP] + 7) >> 3);
          cb *= dev->vbe_regs[VBE_DISPI_INDEX_YRES];
          uint16_t cVirtWidth = dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH];
          if (!cVirtWidth)
            cVirtWidth = dev->vbe_regs[VBE_DISPI_INDEX_XRES];
          if (!cVirtWidth || !dev->vbe_regs[VBE_DISPI_INDEX_YRES] ||
              cb > dev->vram_size) {
          ...
          in BIOS) */
          svga->gdcreg[0x06] = (svga->gdcreg[0x06] & ~0x0c) |
                               0x05; /* graphic mode + memory map 1 */
          ...
          svga->crtc[0x12] = h;
          svga->crtc[0x07] = (svga->crtc[0x07] & ~0x42) | ((h >> 7) & 0x02) |
                             ((h >> 3) & 0x40);
          ...
          svga->gdcreg[0x05] =
              (svga->gdcreg[0x05] & ~0x60) | (shift_control << 5);
      ...
      case VBE_DISPI_INDEX_Y_OFFSET: {
        dev->vbe_regs[dev->vbe_index] = val;
        if (dev->vbe_index == VBE_DISPI_INDEX_X_OFFSET ||
            dev->vbe_index == VBE_DISPI_INDEX_Y_OFFSET) {
          svga_t *svga = &dev->svga;
          if (svga->bpp == 4) {
            svga->rowoffset =
                (dev->vbe_regs[VBE_DISPI_INDEX_VIRT_WIDTH] / 2) >> 3;
            svga->memaddr_latch =
                (dev->vbe_regs[VBE_DISPI_INDEX_Y_OFFSET] * svga->rowoffset) +
                (dev->vbe_regs[VBE_DISPI_INDEX_X_OFFSET] >> 3);
          } else {
          ...
          if (svga->memaddr_latch != dev->memaddr_latch_old) {
            if (svga->bpp == 4) {
              svga->memaddr_backup =
                  (svga->memaddr_backup - (dev->memaddr_latch_old << 2)) +
                  (svga->memaddr_latch << 2);
            } else {
              svga->memaddr_backup =
                  (svga->memaddr_backup - (dev->memaddr_latch_old)) +
                  (svga->memaddr_latch);
...
void bochs_vbe_outl(uint16_t addr, uint32_t val, void *priv) {
  bochs_vbe_outw(addr, val & 0xffff, priv);
  bochs_vbe_outw(addr + 2, val >> 16, priv);
}
void bochs_vbe_out(uint16_t addr, uint8_t val, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  svga_t *svga = &dev->svga;
  uint8_t old;
...
uint8_t bochs_vbe_in(uint16_t addr, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  svga_t *svga = &dev->svga;
  uint8_t ret;
  if (((addr & 0xfff0) == 0x3d0 || (addr & 0xfff0) == 0x3b0) &&
...
static uint8_t bochs_vbe_pci_read(int func, int addr, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  uint8_t ret = 0x00;
  if (func == 0x00)
    switch (addr) {
...
static void bochs_vbe_disable_handlers(bochs_vbe_t *dev) {
...
static void bochs_vbe_pci_write(int func, int addr, uint8_t val, void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  if (func == 0x00)
    switch (addr) {
    case 0x04:
      ...
      if (dev->pci_conf_status & PCI_COMMAND_MEM) {
        ...
        if (dev->pci_rom_enable && (dev->rom_addr != 0x0000) &&
            (dev->rom_addr < 0xfff8))
...
static void bochs_vbe_reset(void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  if (reset_state != NULL) {
...
static void *bochs_vbe_init(const device_t *info) {
...
void bochs_vbe_close(void *priv) {
  bochs_vbe_t *dev = (bochs_vbe_t *)priv;
  ddc_close(dev->ddc);
...
void bochs_vbe_speed_changed(void *priv) {
  bochs_vbe_t *bochs_vbe = (bochs_vbe_t *)priv;
  svga_recalctimings(&bochs_vbe->svga);
}
void bochs_vbe_force_redraw(void *priv) {
  bochs_vbe_t *bochs_vbe = (bochs_vbe_t *)priv;
...
static device_config_t bochs_vbe_config[] = {
    ...
    {.name = "memory",
     ...
     .file_filter = NULL,
     .spinner = {0},
     .selection = {{.description = "128 MB", .value = 128},
